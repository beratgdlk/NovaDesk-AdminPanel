...
export abstract class FileLibraryAssetsService {
  ...
  static async store(data: FileLibraryAssetCreateInput, tx?: Prisma.TransactionClient) {
    try {
      const { file, type, metadata, title } = data;
  ...
  static async update(uuid: string, data: FileLibraryAssetUpdateInput, tx?: Prisma.TransactionClient) {
    try {
      const dbClient = tx || prisma;
