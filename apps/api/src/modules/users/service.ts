...
import { type Gender, Prisma } from "@onlyjs/db/client";
...
export abstract class UsersService {
  ...
  private static async prepareUserPayload(
  ...
  static async index(query?: UserIndexQuery) {
  ...
  static async show(
    ...
    let deletedFilter: {
      deletedAt: Prisma.UserWhereUniqueInput["deletedAt"];
    } | null = {
      deletedAt: null,
  ...
  static async store(payload: UserCreatePayload) {
    try {
      ...
      if (payload.agentUuid !== undefined) {
        if (payload.agentUuid === null) {
          updates.agent = { disconnect: true };
          updates.agentUuid = null;
        } else {
          const agent = await prisma.agent.findUnique({
            where: { uuid: payload.agentUuid },
          });
          ...
          updates.agent = { connect: { id: agent.id } };
          updates.agentUuid = agent.uuid;
        }
      }
  ...
  static async update(id: string, payload: UserUpdatePayload) {
    try {
